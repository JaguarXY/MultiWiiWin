...
namespace MultiWiiWinGUI
{
    ...
    public partial class mainGUI : Form
    {
        ...
        static PointLatLng copterPos = new PointLatLng(30.5067041, 120.7212934);       //Just the corrds of my flying place (25.318183,110.411203)GUET
        ...
        Keys[] KeyData = new Keys[3];
        int kd = 0;
        int[] Drop_WP = new int[3];
        int gpsislost_count = 0;
        ...
        public mainGUI()
        {
            ...
            mapProviders = new GMapProvider[10];
            ...
            mapProviders[6] = GMapProviders.GoogleChinaSatelliteMap;
            mapProviders[7] = GMapProviders.OpenStreetMap;
            ...
            GMOverlayLiveData.Markers.Add(new GMapMarkerCopter(copterPos, 0, 0, 0, 3,0));
            ...
            //MainMap.Refresh();
            attitudeIndicatorInstrumentControl2.ToggleArtificalHorizonType();
            #endregion
        }
...
#region extended_aux_states
        ...
        private void timer_realtime_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (serialPort.BytesToRead == 0)
                {
                    ...
                    if (tabMain.SelectedIndex == GUIPages.Mission && update_cycle_count == 6)
                    {
                        MSPquery(MSP.MSP_DEBUG);
                        MSPquery(MSP.MSP_USE_DEBUG);
                        MSPquery(MSP.MSP_USE_DROP_STATUS);
                    }
        ...
        private void evaluate_command(byte cmd)
        {
            ...
            switch (cmd)
            {
                case MSP.MSP_USE_DEBUG:
                    ptr = 0;
                    mw_gui.u_debug0 = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    mw_gui.u_debug1 = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    mw_gui.u_debug2 = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    mw_gui.u_debug3 = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    mw_gui.u_debug4 = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    mw_gui.u_debug5 = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    mw_gui.u_debug6 = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    mw_gui.u_debug7 = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    break;
                case MSP.MSP_USE_RTH:
                    mw_gui.isReturnHome = true;
                    btn_RTH.ForeColor = Color.LightGreen;
                    break;
                case MSP.MSP_USE_DROP_BOOM:
                    if(mw_gui.is_Dropping == 0)
                        speech.SpeakAsync("Drop Boom!");
                    mw_gui.is_Dropping = 1;
                    break;
                case MSP.MSP_USE_DROP_STATUS:
                    ptr = 0;
                    mw_gui.drop_status = (byte)inBuf[ptr++];
                    break;
                case MSP.MSP_IDENT:
                    ptr = 0;
                ...
                case MSP.MSP_RAW_GPS:
                    ...
                    mw_gui.GPS_heading = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    break;
                ...
                case MSP.MSP_NAV_STATUS:
                    ...
                    mw_gui.across = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    break;
                ...
                case MSP.MSP_NAV_CONFIG:
                    ...
                    mw_gui.drop_delay_ms = BitConverter.ToInt16(inBuf, ptr); ptr += 2;
                    mw_gui.drop_servor_open = BitConverter.ToUInt16(inBuf, ptr); ptr += 2;
                    mw_gui.drop_servor_close = BitConverter.ToUInt16(inBuf, ptr); ptr += 2;
                    break;
        ...
        private void update_gui()
        {
            ...
            if (bOptions_needs_refresh)
            {
                ...
                if (naviGroup.Enabled)
                {
                    ...
                    nWPRadius.Value = mw_gui.wp_radius < nWPRadius.Minimum ? nWPRadius.Minimum : mw_gui.wp_radius;
                    nRTHAlt.Value = mw_gui.rth_altitude < nRTHAlt.Minimum ? nRTHAlt.Minimum : mw_gui.rth_altitude;
                    nCrosstrack.Value = ((decimal)mw_gui.crosstrack_gain / 100) < nCrosstrack.Minimum ? nCrosstrack.Minimum : ((decimal)mw_gui.crosstrack_gain / 100);
                    nMaxSpeed.Value = mw_gui.nav_speed_max < nMaxSpeed.Minimum ? nMaxSpeed.Minimum : mw_gui.nav_speed_max;
                    nMinSpeed.Value = mw_gui.nav_speed_min < nMinSpeed.Minimum ? nMinSpeed.Minimum : mw_gui.nav_speed_max;
                    nBanking.Value = (mw_gui.nav_bank_max / 100) < nBanking.Minimum ? nBanking.Minimum : mw_gui.nav_bank_max / 100;
                    nSafeWPDist.Value = mw_gui.safe_wp_distance < nSafeWPDist.Minimum ? nSafeWPDist.Minimum : mw_gui.safe_wp_distance;
                    nMaxAlt.Value = mw_gui.nav_max_altitude < nMaxAlt.Minimum ? nMaxAlt.Minimum : mw_gui.nav_max_altitude;
                    nLandSpeed.Value = mw_gui.land_speed < nLandSpeed.Minimum ? nLandSpeed.Minimum : mw_gui.land_speed;
                    nFence.Value = mw_gui.fence < nFence.Minimum ? nFence.Minimum : mw_gui.fence;

                    //New refresh 20190727
                    GPSconfig3.Value = mw_gui.drop_delay_ms < GPSconfig3.Minimum ? GPSconfig3.Minimum : (mw_gui.drop_delay_ms > GPSconfig3.Maximum ? GPSconfig3.Maximum : mw_gui.drop_delay_ms);
                    GPSconfig1.Value = mw_gui.drop_servor_open < GPSconfig1.Minimum ? GPSconfig1.Minimum : (mw_gui.drop_servor_open > GPSconfig1.Maximum ? GPSconfig1.Maximum : mw_gui.drop_servor_open);
                    GPSconfig2.Value = mw_gui.drop_servor_close < GPSconfig2.Minimum ? GPSconfig2.Minimum : (mw_gui.drop_servor_close > GPSconfig2.Maximum ? GPSconfig2.Maximum : mw_gui.drop_servor_close);
            ...
            if (tabMain.SelectedIndex == GUIPages.Mission)
            {
                ...
                if (bShowGauges)
                {
                    ...
                    headingIndicatorInstrumentControl2.SetHeadingIndicatorParameters(mw_gui.heading);
                ...
                if (mw_gui.GPS_latitude != 0)
                {
                    ...
                    GMOverlayLiveData.Markers.Add(new GMapMarkerCopter(GPS_pos, mw_gui.heading, 0, mw_gui.target_bearing, mw_gui.multiType, mw_gui.across));
                    ...
                    GPS_Speed.Text = Convert.ToString(mw_gui.GPS_speed);
                    air_Speed.Text = Convert.ToString("...");
                    GPS_Heading.Text = Convert.ToString(mw_gui.GPS_heading);
                    l_Debug1.Text = Convert.ToString(mw_gui.debug1);
                    l_Debug2.Text = Convert.ToString(mw_gui.debug2);
                    l_Debug3.Text = Convert.ToString(mw_gui.debug3);
                    l_Debug4.Text = Convert.ToString(mw_gui.debug4);
                    U_Debug0.Text = Convert.ToString(mw_gui.u_debug0);
                    U_Debug1.Text = Convert.ToString(mw_gui.u_debug1);
                    U_Debug2.Text = Convert.ToString(mw_gui.u_debug2);
                    U_Debug3.Text = Convert.ToString(mw_gui.u_debug3);
                    U_Debug4.Text = Convert.ToString(mw_gui.u_debug4);
                    U_Debug5.Text = Convert.ToString(mw_gui.u_debug5);
                    U_Debug6.Text = Convert.ToString(mw_gui.u_debug6);
                    U_Debug7.Text = Convert.ToString(mw_gui.u_debug7);
                }
            }
            #endregion
            ...
            if (tabMain.SelectedIndex == GUIPages.FlightDeck)
            {
                ...
                //lab_ser1.Text = mw_gui.servos[0].ToString();
                //lab_ser2.Text = mw_gui.servos[1].ToString();
                //lab_ser3.Text = mw_gui.servos[2].ToString();
                //lab_ser4.Text = mw_gui.servos[3].ToString();
                //lab_ser5.Text = mw_gui.servos[4].ToString();
                //lab_ser6.Text = mw_gui.servos[5].ToString();
                //lab_ser7.Text = mw_gui.servos[6].ToString();
                //lab_ser8.Text = mw_gui.servos[7].ToString();
        ...
        //update nav params.
            ...
            mw_params.drop_delay_ms = (short)GPSconfig3.Value;
            mw_params.drop_servor_open = (ushort)GPSconfig1.Value;
            mw_params.drop_servor_close = (ushort)GPSconfig2.Value;
        ...
        void openLog()
        {
            try
            {
                String tempPath = gui_settings.sLogFolder + "\\mwguilog" + String.Format("-{0:yyyyMMdd-hhmm}.log", DateTime.Now);
                tempPath = tempPath.Replace("\\","/");
                wLogStream = new StreamWriter(tempPath);
            }
            catch(Exception ex666)
            {
                MessageBox.Show(ex666+" !!! Unable to open log file at " + gui_settings.sLogFolder + "\\mwguilog" + String.Format("-{0:yyyyMMdd-hhmm}.log", DateTime.Now), "Error opening log", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
                ...
                if (item is GMapMarkerRect) 
                {
                    ...
                    if (item.Tag != null && rc.InnerMarker != null && int.TryParse(rc.InnerMarker.Tag.ToString(), out answer))
                    {
                        try
                        {
                            ...
                            {
                                item.ToolTipText = "Distance: " + (MainMap.MapProvider.Projection.GetDistance(GMRouteMission.Points[answer - 1], GMRouteMission.Points[answer - 2])*1000).ToString("0.0") + "m";
                                //item.ToolTipText = "Distance: " + GetDistance(double.Parse(missionDataGrid[LATCOL.Index, answer - 1].Value.ToString()), double.Parse(missionDataGrid[LONCOL.Index, answer - 1].Value.ToString()), double.Parse(missionDataGrid[LATCOL.Index, answer - 2].Value.ToString()), double.Parse(missionDataGrid[LONCOL.Index, answer - 1].Value.ToString())).ToString() + "m";
                            }
                ...
                if (!isMouseDraging)
                {
                    ...
                    else
                    {
                        if (Control.ModifierKeys == Keys.Control)
                            addWP("WAYPOINT", 0,0,1350, currentMarker.Position.Lat, currentMarker.Position.Lng, iDefAlt);
                        if ((Control.ModifierKeys & Keys.Alt) == Keys.Alt && (Control.ModifierKeys & Keys.Control) == Keys.Control)
                            addWP("WAYPOINT", 0, 0, 1350, center.Position.Lat, center.Position.Lng, iDefAlt);
                    }
                }
                else
                {
        ...
        private void addWP(string action, int P1, int P2, int P3, double Lat, double Lon, int Alt)
        {
            ...
            {
                //绑定投弹点
                Combo_Drop_Boom.Items.Add((selectedrow + 1).ToString());
            }
        ...
        private void updateMap()
        {
            ...
            for (int a = 0; a < missionDataGrid.Rows.Count - 0; a++)
            {
                string sAction = missionDataGrid.Rows[a].Cells[Action.Index].Value.ToString(); //获取任务列表中的值
            ...
            RegenerateMissionRoute(); //连接航点的线条
            lDistance.Text = String.Format("Mission total dist.:{0:N1} m", GMRouteMission.Distance * 1000);
        }
        ...
        private void change_datagrid_header(string sAction)
        {
            if (sAction == "WAYPOINT" || sAction == "POSHOLD_UNLIM")
            {
                ...
                missionDataGrid.Columns[4].HeaderText = "Spe";
            ...
            {
                missionDataGrid.Columns[2].HeaderText = "Sec";
                missionDataGrid.Columns[3].HeaderText = "Rcm";
                missionDataGrid.Columns[4].HeaderText = "Spe";
                missionDataGrid.Columns[5].HeaderText = "Lat";
                missionDataGrid.Columns[6].HeaderText = "Lon";
                missionDataGrid.Columns[7].HeaderText = "Alt";
        ...
        private void b_Record_Click(object sender, EventArgs e)
        {
            if (bVideoConnected)
            {
                if (bVideoRecording == false)
                {
                    ...
                    vfwWriter.Open("video.avi", 640, 480, (int)nFrameRate.Value, (VideoCodec)cb_codec.SelectedIndex, (int)(1000000 * nBitRate.Value));

                    //vfwWriter.Open(gui_settings.sCaptureFolder + "\\Captures" + String.Format("-{0:yyMMdd-hhmm}", DateTime.Now) + ".avi", 640, 480, (int)nFrameRate.Value, (VideoCodec)cb_codec.SelectedIndex, (int)(1000000 * nBitRate.Value));
        ...
        private void cbShowGauges_CheckedChanged(object sender, EventArgs e)
        {
            if (bShowGauges)
            {
                ...
                headingIndicatorInstrumentControl2.Visible = false;
            ...
            else
            {
                ...
                headingIndicatorInstrumentControl2.Visible = true;
        ...
        {
            addWP("WAYPOINT", 0, 0, 1350, start.Lat, start.Lng, iDefAlt);
        }
        ...
        #region 返航
        private void btn_RTH_Click(object sender, EventArgs e)
        {
            if(btn_RTH.ForeColor == Color.White || btn_RTH.ForeColor == Color.LightGreen)
            {
                timer_RTH.Start();
                //speech.SpeakAsync("Return to home!");
                btn_RTH.ForeColor = Color.Red;
            }
        }

        private void timer_RTH_Tick(object sender, EventArgs e)
        {
            if (!mw_gui.isReturnHome && isConnected)
            {
                MSPquery(MSP.MSP_USE_RTH);
            }
            else
            {
                mw_gui.isReturnHome = false;
                timer_RTH.Stop();
            }
        }
        #endregion
        ...
        {
            pictureBox2.Image = global::MultiWiiWinGUI.Properties.Resources.button_RED;
        }

        private void pictureBox2_UP(object sender, MouseEventArgs e)
        {
            pictureBox2.Image = global::MultiWiiWinGUI.Properties.Resources.button_BLUE;
        }

        private void pictureBox2_Click(object sender, EventArgs e)  //投弹按钮
        {
            if(mw_gui.is_Dropping == 0 && isDRB_Start == false)
                timer_DRB.Start();
        }

        private void timer_DRB_Tick(object sender, EventArgs e)     //投弹事件
        {
            if (mw_gui.is_Dropping == 0 && isConnected && Combo_Drop_Boom.Text != "")
            {
                isDRB_Start = true;
                MSPquery_DP(int.Parse(Combo_Drop_Boom.Text));
            }
            else
            {
                mw_gui.is_Dropping = 0;
                isDRB_Start = false;
                timer_DRB.Stop();
            }
        }

        private void DRB_STATUS()
        {
            switch (mw_gui.drop_status)
            {
                case 0:
                    DRB_L1.Visible = false;
                    DRB_L2.Visible = false;
                    DRB_L3.Visible = false;
                    break;
                case 1:
                    DRB_L1.Visible = true;
                    DRB_L2.Visible = false;
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            if(keyData == (Keys.Control | Keys.Enter))
            {
                ...
                Combo_Drop_Boom.Text = Drop_WP[0].ToString();
            else if (keyData == KeyData[1])
                Combo_Drop_Boom.Text = Drop_WP[1].ToString();
            else if (keyData == KeyData[2])
                Combo_Drop_Boom.Text = Drop_WP[2].ToString();
            return base.ProcessCmdKey(ref msg, keyData);
        }
        private void missionGrid_DoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (kd < 3)
            {
                set_Shortcut_Key tipf = new set_Shortcut_Key(e.RowIndex + 1);
                tipf.ShowDialog();
                Drop_WP[kd] = e.RowIndex + 1;
                KeyData[kd++] = tipf.KeyData;
                tipf.Close();
            }
        }
        ...
        private void Combo_Drop_Boom_SelectedIndexChanged(object sender, EventArgs e)
        {
            int current_index = Combo_Drop_Boom.SelectedIndex;
            missionDataGrid.CurrentCell =  missionDataGrid[0,current_index];
        }
        static int _iiii = 0;
        //private void button1_Click(object sender, EventArgs e)
        //{
        //    speech.SpeakAsync(sNavError[_iiii++]);
        //}
        ...
        {
            addWP("SET_POI", 0, 0, 0, start.Lat, start.Lng, int.Parse(txtDefAlt.Text));
        }
