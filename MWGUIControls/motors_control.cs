...
namespace MultiWiiGUIControls
{
    class MWGUIMotors : MWGUIControl
    {
        ...
        enum CopterType { Tri = 1, QuadP, QuadX, BI, Gimbal, Y6, Hex6, FlyWing, Y4, Hex6X, Octo8Coax, Octo8P, Octo8X, Airplane, Heli120,Heli90,Vtail, FixedWing=50 };
        ...
        static int[,] coord_fwing = { { 11, 125, 70 }, { 145, 126, 70 }, { 5, 168, 70 }, { 96, 168, 70 } };
        ...
        Bitmap bmpFixedW = new Bitmap(MultiWiiWinGUI.MWGUIControls.MWGUIControlsResources.fixedwing);
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            switch (CopterTypeToDraw)
            {
                ...
                case CopterType.Heli90:
                    ...
                    break;
                case CopterType.FixedWing:
                    int temp = 3;
                    bmpFixedW.MakeTransparent(Color.Yellow);
                    pe.Graphics.DrawRectangle(maskPen, 0, 0, bmpFixedW.Width, bmpFixedW.Height);
                    pe.Graphics.DrawImageUnscaled(bmpFixedW, 0, 0, bmpFixedW.Width, bmpFixedW.Height);
                    for (i = 0; i < 2; i++)
                    {
                        int h = (int)((motorvals[i] - 900) * (float)(coord_fwing[i, 2] / 1200.0f));
                        pe.Graphics.FillRectangle(drawBrushGreen, coord_fwing[i, 0], coord_fwing[i, 1] - h, 10, h);
                        pe.Graphics.DrawString(String.Format("{0:0}", motorvals[i]), drawFont, drawBrush, coord_fwing[i, 0] - 10, coord_fwing[i, 1]);
                    }
                    for (i = 0; i < 2; i++)
                    {
                        l = (int)((servovals[temp] - 900) * (float)(coord_fwing[2 + i, 2] / 1200.0f));
                        pe.Graphics.FillRectangle(drawBrushGreen, coord_fwing[2 + i, 0], coord_fwing[2 + i, 1] - 10, l, 10);
                        pe.Graphics.DrawString(String.Format("{0:0}", servovals[temp++]), drawFont, drawBrush, coord_fwing[2 + i, 0] + 18, coord_fwing[2 + i, 1] + 8);
                    }
                    break;
