...
namespace MultiWiiWinGUI
{
    ...
    public partial class mainGUI : Form
    {
        ...
        //Do a synchronous msp_query - return false is no answer received within timeout (timeout is in millisec)
        ...
        private void b_connect_Click(object sender, EventArgs e)
        {
            ...
            if (serialPort.IsOpen)              //Disconnect
            {
                ...
                btn_RTH.ForeColor = Color.White;
                mw_gui.isReturnHome = false;
                mw_gui.drop_status = 0;
                gpsislost_count = 0;
                DRB_STATUS();
            }
            else                               //Connect
            {
                ...
                byte tries = 0;
                while (!MSPquery_sync(MSP.MSP_IDENT, 500) && tries < 2) { tries++; }
                if (tries == 2) { failed_connect_cleanup(1); return; }
        ...
        {
            byte c = 0;         //c is CRC checkout
            byte[] o;
            o = new byte[10];   // with checksum
            o[0] = (byte)'$';
            o[1] = (byte)'M';
            o[2] = (byte)'<';
        ...
        private void MSPquery_DP(int wp)
        {
            byte c = 0;         
            byte[] o;
            o = new byte[10];   
            ...
            o[2] = (byte)'<';
            o[3] = (byte)1; c ^= o[3];      
            o[4] = (byte)MSP.MSP_USE_DROP_BOOM; c ^= o[4];
            o[5] = (byte)wp; c ^= o[5];
            o[6] = (byte)c;
